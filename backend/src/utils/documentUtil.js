...
const handleFileUploads = async (files, documentNames = []) => {
  ...
  if (files.vehiclePhotos) {
    uploadedFiles.vehiclePhotos = files.vehiclePhotos.map((photo) => photo.path);
  }

  if (files.farmerDocuments) {
    uploadedFiles.documents = files.farmerDocuments.map((doc, index) => ({
  ...
  if (files.deliveryAgentDocuments) {
    uploadedFiles.documents = files.deliveryAgentDocuments.map((doc, index) => ({
      originalName: doc.originalname,
      documentName: documentNames[index] || doc.originalname,
      fileType: getFileTypeFromMimetype(doc.mimetype),
      size: doc.size,
